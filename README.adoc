...
* `CITIZEN_GCP_GCS_BUCKET`: A GCS bucket to save modules files if `CITIZEN_STORAGE` is `gcs`.
* `GOOGLE_APPLICATION_CREDENTIALS`: Your google application credentials only if `CITIZEN_STORAGE` is `gcs`.
