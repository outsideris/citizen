...
describe('cli-helpers', () => {
  describe('isValidNamespace()', () => {
    ...
    it('should check namespace consists of alphanumeric characters and allow multiple non-adjacent hyphens', () => {
      const result = isValidNamespace('1a-2b-3c');
      expect(result).to.be.true;
    });

    it('should check namespace consists of alphanumeric characters and no double hyphens', () => {
      const result = isValidNamespace('1a2--b3c');
  ...
  describe('isValidName()', () => {
    ...
    it('should check name consists of alphanumeric characters and allow multiple non-adjacent hyphens', () => {
      const result = isValidName('citizen-test-1');
      expect(result).to.be.true;
    });

    it('should check name consists of alphanumeric characters and no double hyphens', () => {
      const result = isValidName('citizen--test1');
      expect(result).to.be.false;
    });

    it('should check name consists of no leading hyphen', () => {
      const result = isValidName('-citizen-test1');
      expect(result).to.be.false;
    });

    it('should check name consists of no trailing hyphen', () => {
      const result = isValidName('citizen-test1-');
      expect(result).to.be.false;
    });
  ...
  describe('makeTarball()', () => {
    ...
    it('should make module files as tarball', (done) => {
      ...
      makeTarball(target, files).then((compressed) => {
        const tarball = new Duplex();
        tarball.push(compressed);
        tarball.push(null);

        tarball.pipe(tar.x({ cwd: UNTAR_DIR })).on('finish', () => {
    ...
    it('should make module filses with .gitignore as tarball', (done) => {
      ...
      makeTarball(target, files).then((compressed) => {
        ...
        tarball.pipe(tar.x({ cwd: UNTAR_DIR })).on('finish', () => {
          recursive(UNTAR_DIR, (err, list) => {
            expect(list).to.have.members([`${UNTAR_DIR}/a.js`, `${UNTAR_DIR}/b.js`]);
            done();
          });
        });
      });
