...
commander
  ...
  .addHelpText(
    ...
    `
Environment variables need to set:
* CITIZEN_STORAGE : storage type, file, s3, minio or gcs
* CITIZEN_STORAGE_PATH : directory to save module files if CITIZEN_STORAGE is file (absolute/relative path can be used).
* CITIZEN_STORAGE_ENDPOINT : storage endpoint to connect private s3 repository if CITIZEN_STORAGE is minio.
