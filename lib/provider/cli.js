...
const { isValidNamespace, isValidName, isValidVersion, isValidProtocol } = require('../cli-helpers');
const { genShaSums, sign, exportPublicKey, publish } = require('./provider');
...
module.exports = async (namespace, type, version, protocols, cmd) => {
  ...
  if (!isValidName(type)) {
    console.error(
      'The type only contain alphanumeric characters or single hyphens, and cannot begin or end with a hyphen.',
    );
    process.exit(1);
  }
  ...
  if (protocols) {
    ...
    if (!protocolVersions.every((p) => isValidProtocol(p))) {
      ...
      console.error(
        'The terraform protocol versions must be comma-separated MAJOR.MINOR format. Examples are 4.1,5.0.',
      );
      process.exit(1);
    }
  }
  ...
  try {
    ...
    const providerRegex = new RegExp(`${escapeRegExp(fileNamePrefix)}_(?<os>.*)_(?<arch>.*)\\.zip`);
    const providerArchives = files.filter((f) => f.match(providerRegex));

    if (providerArchives.length === 0) {
      spinner.fail();
      console.error(
        `There should be at least one zip archive matching file name ${namespace}-${type}_${version}_<os>_<arch>.zip`,
      );
      process.exit(1);
    }
