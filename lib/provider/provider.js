...
const sign = (shaSumsFile, targetDir, gpgKey, passphraseFile) => new Promise((resolve, reject) => {
  let keyOption = '';
  if (gpgKey) {
    keyOption = `--default-key ${gpgKey} `;
  }
  let passphraseFileOption = '';
  if (passphraseFile) {
    passphraseFileOption = `--passphrase-file ${passphraseFile} `;
  }
  exec(`gpg --detach-sign ${keyOption} ${passphraseFileOption} --yes ${shaSumsFile}`, { cwd: targetDir }, (err) => {
    if (err) {
      return reject(err);
    }
