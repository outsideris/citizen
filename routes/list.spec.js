...
describe('GET /v1/modules', () => {
  ...
  it('should return all modules', () =>
    request(app)
      ...
      .expect(200)
      .then((res) => {
        expect(res.body).to.have.property('meta').to.have.property('current_offset').to.equal(0);
...
describe('GET /v1/modules/:namespace/:name/:provider/versions', () => {
  before(async () => {
    await saveModule({
      ...
      version: '1.0.4',
      owner: '',
      definition: { root: { name: 'lb-http' }, submodules: [{ name: 'example' }] },
    });
    await saveModule({
      namespace: 'aws-modules',
      name: 'vpc',
      ...
      version: '1.2.1',
      owner: '',
      definition: { root: { name: 'vpc' }, submodules: [{ name: 'example' }] },
    });
    await saveModule({
      namespace: 'aws-modules',
      name: 'vpc',
  ...
  it('should return root and submodules', () =>
    request(app)
      ...
      .then((res) => {
        ...
        expect(specificModule).to.have.property('root').to.have.property('name').to.equal('vpc');
        expect(specificModule).to.have.property('submodules').to.have.lengthOf(1);
        expect(specificModule.submodules[0]).to.have.property('name').to.equal('example');
      }));
});
